...
public class FindMedianInLargeFile {
     ...
     *
     * The problem is trying to find a guess number between left bound and right bound which can divide all numbers into
     * two sets with same size.
     *
     * @param nums
     * @return
     */
    public double findMedian(int[] nums) {
        ...
        if(length % 2== 0) {
            return (findHelper(nums, Integer.MIN_VALUE, Integer.MAX_VALUE, length / 2 + 1) +
                    findHelper(nums, Integer.MIN_VALUE, Integer.MAX_VALUE, length / 2)) / 2.0;
        }

        return findHelper(nums, Integer.MIN_VALUE, Integer.MAX_VALUE, nums.length / 2 + 1);
    }
